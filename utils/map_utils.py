...
def create_colored_route_map(allocated_df, customers_df, warehouses_df):
    ...
    for i, (truck_id, group) in enumerate(grouped):
        ...
        # Sort customers by route_order
        group_sorted = group.sort_values("route_order")
        points = [start_coord] + list(zip(group_sorted['latitude'], group_sorted['longitude']))

        # Add truck route layer
        ...
        # Distance Labels (including warehouse → first customer)
        ...
        # Customer markers with route order
