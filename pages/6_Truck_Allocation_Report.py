...
if cached_data:
    ...
    st.download_button("⬇️ Download Customer Report ", customer_report_df.to_csv(index=False), file_name="customer_report.csv")
    ...
    if filo_df is not None and not filo_df.empty:
        ...
        if not filtered_filo_df.empty:
            ...
            st.download_button("⬇️ Download FILO Truck Allocation ", filtered_filo_df.to_csv(index=False), file_name=f"filo_truck_allocation_{selected_date}.csv")
...
#         if not filtered_filo_df.empty:
#             # Fix for missing warehouse_id
#             if 'warehouse_id' not in filtered_filo_df.columns:
#                 if 'warehouse_id' in allocation_results.columns:
#                     filtered_filo_df = filtered_filo_df.merge(
#                         allocation_results[['customer_id', 'warehouse_id']],
#                         on='customer_id',
#                         how='left'
#                     )
#                 else:
#                     st.warning("⚠️ 'warehouse_id' not found in allocation_results. Please check allocation logic.")
