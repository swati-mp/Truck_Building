...
if orders_df.empty or customers_df.empty or products_df.empty or trucks_df.empty:
    st.warning("⚠️ Please ensure Customers, Products, Trucks, and Orders are available in the data folder.")
else:
    ...
    if st.session_state.allocation_results is not None:
        ...
        # st.subheader("📑 Truck-wise FILO Loading Manifest")
        # manifest_df = logic.generate_filo_manifest(
        #     st.session_state.filo_allocated_df,
        #     st.session_state.filtered_orders,
        #     customers_df,
        #     products_df
        # )
        ...
        # st.download_button(
        #     "⬇️ Download Loading Manifest",
        #     data=manifest_df.to_csv(index=False),
        #     file_name="filo_manifest.csv",
        #     mime="text/csv"
        # )

        st.subheader("🗺️ Route Map (FILO with Color-coded Trucks)")
        filo_map = map_utils.create_colored_route_map(st.session_state.filo_allocated_df, customers_df)
